...
const migrationPath = "db/migration-090-schema-policy-hardening.sql";
...
describe("migration 090 schema and policy hardening", () => {
  ...
  test("repairs physical drift and advances the schema version", () => {
    ...
    expect(sql).toMatch(/values\s*\(\s*true\s*,\s*90\s*,/i);
  });
