...
describe("migration 091 PostgreSQL behavior", () => {
  ...
  postgresTest(
    ...
    () => {
      ...
      const temp = mkdtempSync(join(tmpdir(), "migration-091-postgres-"));
      ...
      try {
        ...
        psql(join(root, "evals/fixtures/migration-091-fixture.sql"));
        psql(join(root, "db/migration-091-schema-policy-hardening.sql"));
        const rerun = psql(join(root, "db/migration-091-schema-policy-hardening.sql"));
        const assertions = psql(join(root, "evals/fixtures/migration-091-assertions.sql"));
