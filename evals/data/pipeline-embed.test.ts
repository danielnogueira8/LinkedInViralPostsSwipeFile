...
describe("postsNeedingEmbedding pagination (the >1000 backfill-stall bug)", () => {
  test("reads the whole corpus past the PostgREST 1000-row cap", async () => {
    ...
    const posts = Array.from({ length: 2500 }, (_, i) => ({ id: `p${i}`, text: `body ${i}` }));
    dbRef.current = makeFakeSupabase({
  ...
  test("subtracts an already-embedded set that itself exceeds 1000 rows", async () => {
    const posts = Array.from({ length: 2000 }, (_, i) => ({ id: `p${i}`, text: `body ${i}` }));
  ...
  test("does NOT return whitespace-only bodies (they can't be embedded, would re-loop every pass)", async () => {
    dbRef.current = makeFakeSupabase({
      post_embeddings: { rows: [] },
      posts: {
        rows: [
          { id: "real", text: "A genuine post body." },
          { id: "spaces", text: "   " }, // non-null but whitespace-only
          { id: "empty", text: "" },
        ],
      },
    });
