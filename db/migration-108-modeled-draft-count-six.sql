-- Migration 108: widen modeled draft batches from 2-5 to 2-6 drafts.
--
-- The per-turn draft-count cap rose to six, so the persistence layer must
-- accept six-draft batches: the expected_count check on the batch, the
-- slot_index check on its slots (six drafts occupy slots 0-5), and the
-- matching guards inside the claim and checkpoint RPCs.

begin;

alter table public.modeled_draft_batches
  drop constraint if exists modeled_draft_batches_expected_count_check;
alter table public.modeled_draft_batches
  add constraint modeled_draft_batches_expected_count_check
  check (expected_count between 2 and 6);

alter table public.modeled_draft_slots
  drop constraint if exists modeled_draft_slots_slot_index_check;
alter table public.modeled_draft_slots
  add constraint modeled_draft_slots_slot_index_check
