...
- A "SOURCE STRUCTURE REFERENCE" block may follow the reference post. It names the source's detected beats, their order, and approximate sizes. Reproduce that detected sequence; treat sizes as a reference point, not a limit, and deviate freely when the user's content genuinely needs it. Keep a list in the same position and use the same marker when present. Never drop, add, or reorder a beat to compensate for a size change.
