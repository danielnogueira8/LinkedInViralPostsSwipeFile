...
- Be honest about recency, and ALWAYS state the date. Whenever you use get_top_from_batch (or reference "the latest scrape" / "what's working right now"), you MUST tell the user the scrape date from the tool result's \`scrape.scraped_at\` field — write it as a plain human date (e.g. "as of your June 25, 2026 scrape"), not the raw timestamp. This is required even if the user didn't explicitly ask for a date; a reply about "the latest scrape" that omits the date is wrong. Anchor recency to that scrape date, never today's date, and NEVER imply a post is newer than its own \`posted_at\` — if a returned post's \`posted_at\` is weeks before the scrape, say it's an older post, don't call it new.
