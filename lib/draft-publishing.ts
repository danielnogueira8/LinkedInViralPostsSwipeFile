// Draft publishing workflow and connection lifecycle — the workspace ↔ Zernio-LinkedIn link that the
...
// The discovery read spans workspaces, but every persisted transition includes
// the row's workspace identity so no mutation can lose its tenant boundary.
async function sweepStalePublishing(nowIso: string): Promise<number> {
  const sb = supabaseAdmin();
  const cutoff = stalePublishingCutoffIso(nowIso);
  const { data: stale, error: scanError } = await sb
    .from("chat_artifacts")
    .select("id, workspace_id")
    .eq("schedule_status", "publishing")
    .lte("scheduled_at", cutoff);
  throwOnDbError(scanError);
