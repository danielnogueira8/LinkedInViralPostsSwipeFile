...
export function handleFromUrl(url: string): string {
  ...
  if (!normalized) return url.toLowerCase();
  const handle = normalized.split("/in/")[1];
  return handle ? handle.toLowerCase() : url.toLowerCase();
}

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

function parseDay(d: string | undefined, time: "00:00:00.000Z" | "23:59:59.999Z"): string | null {
  if (!d || !DATE_RE.test(d)) return null;
  const dt = new Date(`${d}T00:00:00.000Z`);
  if (Number.isNaN(dt.getTime()) || dt.toISOString().slice(0, 10) !== d) {
    return null;
  }
  return new Date(`${d}T${time}`).toISOString();
}

export function parseDayStart(d?: string): string | null {
  return parseDay(d, "00:00:00.000Z");
}

export function parseDayEnd(d?: string): string | null {
  return parseDay(d, "23:59:59.999Z");
}
