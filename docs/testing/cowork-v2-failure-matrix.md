...
| Hidden legacy prepass failure | decision usage/telemetry, freshness cache-miss, backstory telemetry, and exemplar-retrieval tests | Paid decision, freshness, backstory, and exemplar-embedding calls share the adapter boundary; provider/content failures remain visible while authoritative usage failures stop the turn. Backstory caches no-facts only after a schema-valid empty response. One query embedding is reused for both exemplar RPCs. |
