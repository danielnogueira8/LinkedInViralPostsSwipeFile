...
import { completeChat, BACKGROUND_MODEL, logOpenRouterUsage } from "@/lib/openrouter";
...
export async function POST(_req: Request, { params }: Ctx) {
  ...
  try {
    ...
    const { text, usage, model: servedModel } = await completeChat({
      // Tiny mechanical job → the cheap background tier (Haiku 4.5 under the
      // Anthropic flag; GLM off it).
      model: BACKGROUND_MODEL,
      maxTokens: 24,
      // A title is a tiny mechanical output. Do not let reasoning consume its
      // 24-token ceiling; the override is ignored on non-GLM models.
      glmReasoning: "none",
      messages: [
    ...
    const titleAttribution = providerModelAttribution(BACKGROUND_MODEL, servedModel);
